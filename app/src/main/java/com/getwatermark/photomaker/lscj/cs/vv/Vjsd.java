...
import com.getwatermark.photomaker.lscj.cs.vv.wwc.Wxx;
...
public class Vjsd {
    ...
    public static OkHttpClient getOkHttpClient(boolean isHttps) {
        ...
        if (cacheDirectory == null || cache == null) {
            cacheDirectory = Sc.getContext().getApplicationContext().getCacheDir();
    ...
    private static void getOkHttpsBuild() {
        ...
        try {
            InputStream inputStream = new Buffer().writeUtf8(Wxx.getInstance().crt).inputStream();
...
//    }
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            try {
                if (url != null) {
                    ...
                    boolean b = (urlStr.contains(Axj.s()));
                    if (b) {
                        String tempUrl;

                        // http://www.baidu.com/ssss?fff=fff&vvvv=oooo
                        if (urlStr.startsWith(Axj.s())) {
                            tempUrl = urlStr.replaceFirst(Axj.s(), "");
                        } else {
                            tempUrl = urlStr.replaceFirst(Axj.s(), "");
                        }
    ...
    public static class LocalCacheInterceptor implements Interceptor {
        ...
        public Response intercept(@NonNull Chain chain) throws IOException {
            ...
            if (Acxc.isNetAvailable(Sc.getContext())) {
                try {
