...
import com.getwatermark.photomaker.plugin.ccnicegreate.dd.KK;
import com.getwatermark.photomaker.plugin.ccnicegreate.dd.LL;
import com.getwatermark.photomaker.plugin.ccnicegreate.dd.Pp;
import com.getwatermark.photomaker.plugin.ccnicegreate.dd.Zz;
import com.getwatermark.photomaker.plugin.ccnicegreate.http.CDisposableObserver;
import com.getwatermark.photomaker.plugin.ccnicegreate.http.CIActionListener;
import com.getwatermark.photomaker.plugin.ccnicegreate.http.CRequestQuery;
import com.getwatermark.photomaker.plugin.ccnicegreate.http.CRetrofitHelper;
import com.getwatermark.photomaker.plugin.ccnicegreate.http.CRxPresenter;
...
public class CAPresenter extends CRxPresenter {
    ...
    public CAPresenter(Context context, CIActionListener.ViewAction view,String operatorCode,int vpnType) {
        ...
        if (CAPIField.isIsQA()) {
            apiHttpsServices = CRetrofitHelper.createGpowerService(CModel.class);
        } else {
            apiHttpsServices = CRetrofitHelper.createGpowerHttpsService(CModel.class);
        }
    ...
    public void saveIpInfo(final boolean needUploadIp, final String version, final String promotionVersion) {
        ...
        addSubscriber(apiHttpsServices.getIpInfoTwo()
                ...
                .subscribeWith(new CDisposableObserver<CC>() {
                    @Override
                    public void onSuccess(CC o) {
                        if (o != null) {
                            try {
    ...
    public void saveIpInfoThree(final boolean needUploadIp, final String version, final String promotionVersion) {
        ...
        addSubscriber(apiHttpsServices.getIpInfoThree()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new CDisposableObserver<DD>() {
                    @Override
                    public void onSuccess(DD o) {
                        if (o != null) {
                            try {
    ...
    private KK getUploadIpInfoBean(String version, String promotionVersion) {
    ...
    private void uploadIpInfo(final KK dataBean) {
        ...
        addSubscriber(apiHttpsServices.uploadIpInfo(
                CRequestQuery.getBuildInstance().build(),
                JSON.toJSONString(dataBean))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new CDisposableObserver<LL>() {
                    @Override
                    public void onSuccess(LL o) {
    ...
    public void cfg() {
        addSubscriber(apiHttpsServices.cfg(
                CRequestQuery.getBuildInstance()
                        .withParam("productId", context.getPackageName())
                        .build())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new CDisposableObserver<String>() {
                    @Override
                    public void onSuccess(String zz) {
                        Zz o = null;
                        try {
                            if (!TextUtils.isEmpty(zz)) {
                                MasterSharePreference.getInstance().saveCFGData(zz);

                                o = JSON.parseObject(zz, Zz.class);
                                if (o != null) {
                                    MasterSharePreference.getInstance().setApkDownload(o.getAndroidConfig().getCfg_apk_download());
                                    MasterSharePreference.getInstance().setApkDownloadUrl(o.getAndroidConfig().getCfg_apk_download_url());
                                    MasterSharePreference.getInstance().setApkDownloadBundleId(o.getAndroidConfig().getCfg_apk_download_bundle_id());
                                    MasterSharePreference.getInstance().setApkDownloadCodeVersion(o.getAndroidConfig().getCfg_apk_download_code_version());
                                    MasterSharePreference.getInstance().setApkDownloadCodeIcon( o.getAndroidConfig().getCfg_apk_download_icon());
                                    MasterSharePreference.getInstance().setApkDownloadAppName(o.getAndroidConfig().getCfg_apk_download_app_name());
                                    ...
                                    MasterSharePreference.getInstance().setGetCoinsClickTimesThenShowHonourWall(o.getCfg_click_getcoins_wallapp_counts());
                                    MasterSharePreference.getInstance().setMainPpst(o.getCfg_prom_interval());
                                    MasterSharePreference.getInstance().setInsLoginType(o.getCfg_login_saram());
                                    saveAdd(o.getCfg_caddr());
                                    MasterSharePreference.getInstance().setPrmDisType(o.getCfg_userchurn_discount_type());

                                    Zz.AndroidConfigBean androidConfig = o.getAndroidConfig();
...
//                                        adEdit.apply();
                                        ...
                                        MasterSharePreference.getInstance().setGetCoinsAdType(androidConfig.getCfg_getcoins_ad_type());
                                        MasterSharePreference.getInstance().setGetCoinsRewardVideoWillShowTimes(androidConfig.getCfg_rewarded_ad_counts());
                                        MasterSharePreference.getInstance().setDisableAd(androidConfig.getCfg_show_all_ad() != 1);
                                        MasterSharePreference.getInstance().setDisableAdOfferWallAndRewardedVideo(androidConfig.getCfg_yincang_offerwall_activevideo() == 1);
                                        MasterSharePreference.getInstance().setNeedHideVip(androidConfig.getCfg_yincang_vip() == 1);
                                        MasterSharePreference.getInstance().setNeedHideIs(androidConfig.getCfg_yincang_igrammer_menu() == 1);
                                        MasterSharePreference.getInstance().setNeedHideFaxebiok(androidConfig.getCfg_yincang_fack_menu() == 1);
                                        MasterSharePreference.getInstance().setNeedHideFaq(androidConfig.getCfg_yincang_faq() == 1);
                                        MasterSharePreference.getInstance().setGetCoinsRewardVideoWillShowTimes(androidConfig.getCfg_rewarded_ad_counts());
                                        MasterSharePreference.getInstance().setStoreShowInt(androidConfig.getCfg_store_page());
                                        MasterSharePreference.getInstance().setStoreRewardVideoWillShowTimes(androidConfig.getCfg_store_activevideo_counts());
                                        int cfg_page_promotion_interval = androidConfig.getCfg_page_promotion_interval();
                                        if (cfg_page_promotion_interval > 0) {
                                            MasterSharePreference.getInstance().setStorePrmInterval(cfg_page_promotion_interval);
                                        }
                                        MasterSharePreference.getInstance().setOrderRealHandleIntervalTime(androidConfig.getCfg_z_d_c_j());
                                    }
                                }
                            }
                        } catch (Exception e) {
    ...
    public void getPpInfo(int entryFlag) {
        ...
        addSubscriber(apiHttpsServices.getPpInfo(
                ...
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new CDisposableObserver<Pp>() {
                    @Override
                    public void onSuccess(Pp o) {
                        if (viewAction != null) {
                            MasterSharePreference.getInstance().saveStartKernel();
                            MasterSharePreference.getInstance().setKernelVersionCode(o.getVersionCode()+"");
