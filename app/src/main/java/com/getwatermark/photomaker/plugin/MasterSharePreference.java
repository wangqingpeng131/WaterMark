...
public class MasterSharePreference {
    public static final String PREFERENCE = "base_" + "KkKkKk" + "_SharePreference";
    private static MasterSharePreference instance = null;

    private SharedPreferences mPreference;

    public MasterSharePreference(String name) {
      if (PCache.getContext() != null) {
            try {
                if (TextUtils.isEmpty(name)) {
                    name = PREFERENCE;
                }
                mPreference = PCache.getContext().getSharedPreferences(name, Context.MODE_PRIVATE);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public MasterSharePreference() {
        this(PCache.getContext().getPackageName());
    }

    public static MasterSharePreference getInstance() {
        if (instance == null) {
            instance = new MasterSharePreference();
