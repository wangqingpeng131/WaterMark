...
import com.getwatermark.photomaker.adapter.ItDecoration
...
class EdActivity : BaseActivity(), TemplateCallback {
        ...
        selector_group.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.water_mark_selector -> {
                    ...
                    edit_select_rv.addItemDecoration(ItDecoration(16.dipToPx()))
                }
                R.id.color_selector -> {
                    ...
                    edit_select_rv.removeItemDecorationAt(0)
                    edit_select_rv.addItemDecoration(ItDecoration(32.dipToPx()))
                }
            }
        }
    ...
    companion object {
        ...
        fun callingIntent(context: Context, imageUri: Uri): Intent {
            val intent = Intent(context, EdActivity::class.java)
    ...
    override fun template(v: View?, position: Int) {
        when (v?.id) {
            R.id.item_water_mark_select_bg -> {
                ...
                Log.i("topwise","isChange1: $isChange")
